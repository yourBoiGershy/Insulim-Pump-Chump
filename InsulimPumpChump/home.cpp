...
home::home(QWidget *parent, const Profile &activeProfile) :
    QWidget(parent),
    ui(new Ui::home),
...
void home::logEvent(const QString &event){
    QString time = QTime::currentTime().toString("hh:mm");
    QListWidgetItem *item = new QListWidgetItem(QString("%1 : %2").arg(time).arg(event));
...
void home::receiveInsulin(double units) {
    ...
    QString label = QString("%1 units administered").arg(units);
    logEvent(label);
...
void home::receiveCarbs(double carbs){
    carbsOnBoard += carbs;
}

void home::scheduleExtendedBolus(double extendedBolusTotal, double durationHours) {
    extendedStartDelayTicks = 1;
    extendedTicksRemaining = durationHours * 12;
    extendedBolusRemaining = extendedBolusTotal;
    extendedBolusPerTick = extendedBolusTotal / extendedTicksRemaining;
    extendedBolusScheduled = true;
}
...
void home::generateGlucoseData() {
    ...
    double targetGlucose = profile.getCurrentTargetGlucose();
    double correctionFactor = profile.getCurrentCorrectionFactor();
    double basalRate = profile.getCurrentBasalRate();

    // predict glucose every 30 mins
    double predicted = predictGlucose30min();

    // Random fluctuation with some physiological constraints
    double randomFactor = (QRandomGenerator::global()->generateDouble() * 0.4) - 0.2;

    if (extendedBolusScheduled) {
        if (extendedStartDelayTicks > 0) {
            extendedStartDelayTicks--;
        } else if (extendedTicksRemaining > 0) {
            insulinOnBoard += extendedBolusPerTick;
            extendedBolusRemaining -= extendedBolusPerTick;
            extendedTicksRemaining--;

            if (extendedTicksRemaining == 0) {
                extendedBolusScheduled = false; // All done
            }
        }
    }
    ...
    if (insulinOnBoard > 0) {
        if (predicted < 4.0) {
            // Full insulin suspension
            insulinEffect = 0;
            // simulate recovery
            double recovery = qMin(0.3, (4.0 - currentGlucose) * 0.6);
            currentGlucose += recovery;
            setBolusIndicatorColour("stopped");
            logEvent("Stopped insulin");
        } else if (predicted < targetGlucose) {
            // Reduced insulin effect
            insulinEffect = -0.015 * insulinOnBoard;
            setBolusIndicatorColour("decrease");
            logEvent("Decreased insulin");
        } else if (predicted > 9.0) {
            // Auto correction and normal basal
            insulinEffect = -0.02 * insulinOnBoard;
            if (!simulationRunning) {
                double correction = qBound(0.0, ((predicted - targetGlucose) / correctionFactor), 1.0);
                insulinOnBoard += correction;
                simulationRunning = true;
                ui->PauseResumeBtn->setText("Pause");
            }
            setBolusIndicatorColour("increase");
            logEvent("Increase insulin");
        } else {
            // Normal basal effect
            insulinEffect = -basalRate * insulinOnBoard;
            setBolusIndicatorColour("maintain");
        }

        // Deplete insulin
        insulinOnBoard = qMax(0.0, insulinOnBoard - 0.02);
    }

    double carbEffect = 0;
    if (carbsOnBoard > 0.0) {
        carbEffect = 0.3 * carbsOnBoard;
        carbsOnBoard = qMax(0.0, carbsOnBoard - 0.4);
    }

    // Calculate new glucose level
    currentGlucose += randomFactor + insulinEffect + carbEffect;
...
double home::predictGlucose30min() {
    double projected = currentGlucose;

    double projectedInsulin = qMax(0.0, insulinOnBoard - 0.02 * 30);
    double projectedInsulinEffect = -0.05 * projectedInsulin;

    double projectedCarbs = qMax(0.0, carbsOnBoard - 0.05 * 30);
    double projectedCarbEffect = 0.1 * projectedCarbs;

    return projected + projectedInsulinEffect + projectedCarbEffect;
}

void home::setBolusIndicatorColour(const QString &state) {
    QString color;
    QString tooltip;

    if (state == "increase") {
        color = "blue";
        tooltip = "Basal increased (auto correction)";
    } else if (state == "decrease") {
        color = "orange";
        tooltip = "Basal decreased (low prediction)";
    } else if (state == "stopped") {
        color = "red";
        tooltip = "Insulin delivery suspended";
    } else {
        color = "gray";
        tooltip = "Basal maintained";
    }

    ui->BolusIndicator->setStyleSheet("color: " + color + "; font-weight: bold; font-size: 20px;");
    ui->BolusIndicator->setToolTip(tooltip);
}
